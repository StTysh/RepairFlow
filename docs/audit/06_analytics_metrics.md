...
## Severity counts
CRITICAL: 2 · HIGH: 2 · MEDIUM: 2 · LOW: 1 · NIT: 2

## Verdict
The numbers **do not currently reconcile** across screens for anything money-shaped: Property History/Stats and Insights/Reports/CSV compute "quoted" spend from two entirely different tables that are never kept in sync, so the same case shows different totals on different pages — by design gap, not by a rare edge case. Within a single family (Insights ↔ Reports ↔ CSV, all backed by `case_detail_rows`), the numbers do reconcile exactly, in pence, including negative adjustments — that part is solid. Largest-remainder percentages and the zero-baseline period comparison are both correctly implemented with no counter-example found.
...
### 2. CRITICAL — Property `/history` and `/stats` include archival sample data unconditionally, with no `include_archived` parameter and no disclosure field
...
Verified: seeded 1 real ACTIVE case + 2 archival RESOLVED cases at one property →
`load_property_stats(...).total_count == 3`, `active_count == 1`. The property page's headline "3 cases" figure is 33%-fabricated from synthetic sample history with no way for the UI or a user to know that, and no query parameter to exclude it. `load_property_history` returns all 3 rows the same way, each with no `is_archived` flag (contrast `analytics.CaseDetailRow.is_archived`, which exists specifically so Insights/Reports/CSV can label this honestly).

**Smallest correct fix:** add `include_archived: bool = True` to both functions' signatures (mirroring every function in `analytics.py`), filter `archive_batch_id IS NULL` when `False`, and add `is_archived` to `PropertyHistoryItem` plus `archived_case_count`/`includes_archived_history` to `PropertyStatsResponse`/`PropertyHistoryResponse`.

### 3. HIGH — `resolution_time_distribution`'s archival branch has no status filter: CANCELLED archival cases are averaged into "resolution time"
...
### 4. MEDIUM — `resolution_hours`'s silent `updated_at` fallback is real, and `skipped_count` does not catch it
`backend/app/analytics.py:436-462`: when a real case has `status == RESOLVED` but no `CASE_RESOLVED`/`CASE_CANCELLED` `CaseEvent` on record, `end = case.terminal_event_at or case.updated_at` silently substitutes `updated_at` — which is "whenever the row was last touched," not a resolution time — and returns a normal-looking float. `skipped_count` (`analytics.py:678`, incremented at `resolution_time_distribution` line 718) only increments when `resolution_hours()` returns `None`, i.e. only for the negative-duration case; the fallback path returns a value, so it is never counted as skipped and is indistinguishable downstream from a genuine event-derived duration.

Verified mechanically: fixture Case B (RESOLVED, `created_at` 40 days ago, `updated_at` 1 day ago, zero `CaseEvent` rows) → `case_detail_rows` reports `resolution_hours = 936.0` and `resolution_time_distribution` counts it in `sample_count` (contributing to `average_hours`), not in `skipped_count` (which stayed at 1, entirely from the unrelated negative-duration Case C).

Negative-duration exclusion itself is correctly implemented — verified Case C (terminal event 1h before `created_at`) returns `None` and is excluded from `sample_count`/`average_hours`/buckets, not clamped to 0.

**Likely real-world frequency (SUSPECTED, not measured against live traffic):** the only code path that sets `status = RESOLVED` (`services.py:955-963`) writes the `CASE_RESOLVED` event atomically in the same function/transaction, so under normal operation this gap should rarely trigger today. It remains a live risk for any future write path, data migration, or manual DB fix that flips status without going through that function, and the audit brief specifically asked for this to be checked, so it's reported as CONFIRMED-mechanism / SUSPECTED-frequency.

**Smallest correct fix:** have `resolution_hours`/`ResolutionInputs` return a tri-state (e.g. a separate `is_estimated: bool`) instead of overloading `updated_at` as if it were authoritative, and count the fallback case in a new `estimated_count` (distinct from `skipped_count`) so the UI can caveat it.
