...
export const Route = createFileRoute("/maintenance/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "All property issues, from report to resolution. Track open, in-progress and resolved maintenance tickets." },
      ...
      <div className="px-8 py-6">
        <header className="flex items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">All property issues, from report to resolution.</p>
          </div>
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      isCase ? "bg-selected outline outline-2 -outline-offset-2 outline-selected-ring" : "hover:bg-muted/60",
                    )}
                  >
                    <td className="px-4 py-3 text-muted-foreground">
                      {isCase ? (
                        <Link to="/maintenance/tickets/$ticketId/{-$section}" params={{ ticketId: "1042", section: undefined }} className="block">#{t.id}</Link>
                      ) : `#${t.id}`}
                    </td>
