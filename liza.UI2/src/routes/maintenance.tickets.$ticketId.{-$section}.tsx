...
export const Route = createFileRoute("/maintenance/tickets/$ticketId/{-$section}")({
  ...
  head: ({ loaderData }) => {
    const title = loaderData ? `#1042 – Roof leak${loaderData.section ? ` · ${cap(loaderData.section)}` : ""} — Fixi` : "Ticket not found — Fixi";
    return {
      meta: [
        { title },
        { name: "description", content: "Case details for the roof leak at 14 King Street, Walthamstow: agent timeline, latest messages and property history." },
        { property: "og:title", content: title },
        { property: "og:description", content: "Case details, AI agent timeline and latest messages for ticket #1042." },
      ],
    };
  },
  component: CasePage,
    ...
    <AppShell>
      <div className="px-8 py-6">
        <div className="flex items-center justify-between">
          <Link to="/maintenance" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        ...
        <div className="mt-4 flex items-start justify-between gap-6">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold tracking-tight">#{c.id} – {c.issue}</h1>
          ...
          <ol className="flex items-start">
            {c.steps.map((s, i) => (
              <li key={s.label} className="relative flex flex-1 flex-col items-center text-center">
                ...
                <div className={cn("mt-2 text-xs font-medium", s.state === "future" ? "text-muted-foreground" : "text-foreground")}>
                  {s.label}
                </div>
        ...
        <div className="mt-5 flex items-center gap-1.5">
          <SectionLink section={undefined} active={section === null}>All</SectionLink>
          <SectionLink section="summary" active={section === "summary"}>Summary</SectionLink>
          <SectionLink section="timeline" active={section === "timeline"}>Timeline</SectionLink>
          <SectionLink section="messages" active={section === "messages"}>Messages</SectionLink>
        </div>

        <div className={cn("mt-3 grid gap-4", section === null ? "xl:grid-cols-3" : "xl:grid-cols-1")}>
...
function ToolbarButton({ icon: Icon, children }: { icon: typeof Share2; children: React.ReactNode }) {
  return (
...
function SectionLink({ section, active, children }: { section?: Section | undefined; active: boolean; children: React.ReactNode }) {
  return (
    <Link
      ...
      className={cn(
        ...
        active ? "border-foreground bg-foreground text-background" : "border-border bg-card text-foreground hover:bg-accent",
      )}
    >
      {children}
    </Link>
...
function StepDot({ state }: { state: "done" | "current" | "future" }) {
  ...
  return <span className="relative z-10 h-5 w-5 rounded-full border-2 border-timeline-future bg-card" />;
}
...
function Avatar({ initials, tone }: { initials: string; tone: "gray" | "green" | "purple" }) {
  ...
  return (
    <div className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-semibold", cls)}>
      {initials}
    </div>
  );
}
    ...
    <Card className="p-5">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        {[ceilingStain, houseExterior, ceilingDamp].map((src, i) => (
          <img key={i} src={src} alt="" loading="lazy" width={912} height={736} className="aspect-[4/3] w-full rounded-lg object-cover" />
        ))}
      </div>
...
function TimelineColumn() {
  return (
    <Card className="p-5">
      <SectionHeader title="Agent timeline" subtitle="What the AI agent has done and what's next." />
      <ol className="mt-4">
            ...
            <li key={item.title} className="relative flex gap-3 pb-5 last:pb-0">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div className={cn("text-[13px] font-semibold", item.state === "future" && "text-muted-foreground")}>
                    {item.title}
                  </div>
                  {item.time ? (
          ...
          return (
            <li key={i} className="flex gap-3 py-3 first:pt-0">
              {isAI ? (
                ...
                <Avatar initials={m.type === "Tenant" ? "JD" : "AR"} tone={m.type === "Tenant" ? "gray" : "green"} />
              )}
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[13px] font-semibold">{m.sender}</span>
                    <Pill tone={isAI ? "purple" : m.type === "Tenant" ? "gray" : "green"} className="ml-2 text-[10px]">
                      {m.type}
                    </Pill>
                  </div>
                ...
                {m.attachments === "tenant" && (
                  <div className="mt-2 flex gap-1.5">
                    {[ceilingStain, ceilingDamp].map((src, j) => (
                      <img key={j} src={src} alt="" loading="lazy" width={912} height={736} className="h-14 w-20 rounded-md object-cover" />
                    ))}
                  </div>
                )}
                {m.attachments === "contractor" && (
                  <div className="mt-2 flex gap-1.5">
                    <img src={roofFlashing} alt="" loading="lazy" width={912} height={736} className="h-14 w-20 rounded-md object-cover" />
                  </div>
                )}
              </div>
            </li>
