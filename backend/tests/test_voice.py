...
def _clear_elevenlabs_env() -> None:
    # Real env vars take priority over backend/.env in pydantic-settings'
    # source order, so an empty-string override (not os.environ.pop, which
    # is a no-op against a key that only ever lived in the .env file) is
    # what actually makes elevenlabs_live False regardless of what real
    # credentials a developer happens to have locally.
